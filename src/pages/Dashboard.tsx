...
export default function Dashboard() {
  ...
  const processFile = async () => {
    ...
    const user = auth.currentUser;
    if (!user) {
      setError("Authentication error: You must be logged in to analyze documents.");
      return;
    }

    // Safety check - if userStatus is still null, we bypass the limit temporarily.
    ...
    try {
      ...
      await fetchData(); // Refresh status and history
